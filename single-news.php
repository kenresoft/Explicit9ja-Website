...
$url = $_REQUEST['e-news'];
$query = "select tblnews.id, tblnews.likes, tblnews.PostTitle as posttitle,tblnews.PostImage,tblcategory.CategoryName as category,tblcategory.id as cid,tblsubcategory.Subcategory as subcategory,tblsubcategory.SubCategoryId as sid,tblnews.PostDetails as postdetails,tblnews.PostingDate as postingdate,tblnews.PostUrl as url from tblnews left join tblcategory on tblcategory.id=tblnews.CategoryId left join  tblsubcategory on  tblsubcategory.SubCategoryId=tblnews.SubCategoryId where tblnews.PostUrl='$url'";
...
if (!isset($_SESSION['recent_posts'][$id])) { mysqli_query($con,"UPDATE `tblnews` SET `views` = `views` + 1 WHERE `id`= $id"); $_SESSION['recent_posts'][$id] = 1; }
// unset($_SESSION['recent_posts'][$id]); ?>
...
<head>
	    ...
	    <!-- Title -->
    <title>Explicit9ja.com | <?php echo $row['posttitle']; ?></title>
...
<li><div>
                        ...
                        <div id="tutorial-<?php echo $res["id"]; ?>">
                            ...
                            <?php
                            $query ="SELECT * FROM tblnews_likes_map WHERE news_id = '" . $res["id"] . "' and ip_address = '" . $ip_address . "'";
                    ...
                    <!--post share end-->
                        ...
                        <div class="comment_area mb-50 clearfix">
                            <h5 class="title">
                                ...
                                if ($total_rows == 0) {
                                    echo 'No comments on this news post yet';
                ...
                <div class="col-12 col-sm-6 col-lg-4 razo-blog-masonary-item mb-80">
                    <div class="razo-blog-masonary-single-item">
                        ...
                        <div class="post-thumbnail">
                            <a href="single-news.php?e-news=<?php echo htmlentities($row['url'])?>"><img src="admin/postimages/<?php echo htmlentities($row['PostImage']);?>" alt=""></a>
                        </div>
                        <!-- Post Content -->
                        <div class="post-content">
                          ...
                          <a href="single-news.php?e-news=<?php echo htmlentities($row['url'])?>" class="post-title"><?php echo $row['posttitle'];?></a>
                            <p><?php $pt=$row['postdetails']; echo (substr($pt,0,200));?><b>...</b></p>
                            <a href="single-news.php?e-news=<?php echo htmlentities($row['url'])?>" class="btn read-more-btn">Read More <i class="fa fa-angle-double-right" aria-hidden="true"></i></a>
                        </div>
                    </div>
                </div>
